...
" "abcd")))
 ...
 (lisp-mode (@ (@ (dict :x 1)
            ...
            (defpackage (defpackage :walk-alone (:import-from :local-time)
                                     (:import-from :hunchentoot)
                                     (:use :cl :alexandria :serapeum)))
            (flatten (flatten '(a b c d (e f))))
            (function-name (function-name #'+))
            (hiding-exists (comment "What is implied by X changes based on whether I want anything implied by X.")
                           (declare (X implies Y))
                           (declare (i want Y))
                           (hiding-exists)
                           (ask (does X imply Y))
                           ...
                           (ask (does X imply Y))
                           (hiding-exists))
            (inline-css (inline-css '(:margin "10 0 0 5"))
            ...
            (partial (funcall (partial #'+ 1 2)
                              3))
            (ps (ps (defun greeting-callback nil (alert "Hello World"))))
            (recall-facts (declare (i want X)))
            (remove (remove "a" '(("a" . b)
                                  ("c" . d))
                            :key #'car :test #'equal))
            (window-by-number (window-by-number 0))
            (with-html-output-to-string (let ((hello-string "Hello World")
                                              (repeat-count 10))
                                          (with-html-output-to-string (s)
                                                                      (:html (:head (:title "Parenscript tutorial: 1st example"))
                                                                             (:body (:h2 :style (inline-css '(:color blue))
                                                                                          "Parenscript tutorial: 1st example")
                                                                                    "Please click the link below." :br (loop repeat repeat-count do (htm (:div :style (inline-css '(:margin "10 0 0 5"))
                                                                                                                                                                (:a :href "#" :onclick (ps (alert (lisp hello-string)))
                                                                                                                                                                     (str hello-string)))))))))))
